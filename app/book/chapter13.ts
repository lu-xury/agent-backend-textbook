...
export const chapter13: Chapter = {
  ...
  opening: [
    "Agent 后端不是把模型调用塞进 while 循环。模型输出具有概率性，工具会改变外部世界，用户会取消请求，网络会超时，服务会重启。可靠的 Agent Runtime 必须把这些不确定性写成可校验的状态、事件、权限和恢复规则。",
    "这一章把前十二章的知识收束到同一个系统：会话和 Run 如何建模，模型候选调用如何验证，工具如何授权与审计，长任务怎样取消、恢复和评估。",
  ],
  goals: [
