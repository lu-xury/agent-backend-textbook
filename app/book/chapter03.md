后端程序看起来是在“处理一个请求”，但这个请求并不是一段直接从浏览器跳到业务代码的文字。它先要把域名解析成地址，在网络中找到机器；操作系统再把到达的数据交给某个端口；连接上的字节经过 TLS 解密，HTTP 服务器才把它解释成方法、路径、请求头和消息体。理解这条链路的目的不是背协议缩写，而是能回答三个实际问题：请求为什么会慢或失败？客户端重试会不会造成重复操作？一项安全配置究竟保护了什么、没有保护什么？
...
把它背成“客户端两次、服务端一次”没有价值。更有用的理解是：若只有两步，服务端无法确认自己的 SYN-ACK 到达客户端，旧的延迟 SYN 也更容易诱导服务端保留无效状态。三次不是让所有失败消失；半开连接、SYN 洪泛和握手超时仍需内核及入口设备处理。应用程序观察到的 `connect timeout` 往往意味着在给定时间内没有完成这个过程，也可能是网络过滤或目标不可达，并不自动说明服务端应用宕机。
...
对于一次写操作超时，最重要的第一步通常不是立即重试，而是判断业务结果：用幂等键查已保存的结果、查询操作状态，或由服务端以事务性记录确认。对于 SSE/WebSocket 断流，检查代理是否缓冲、空闲超时是否一致、是否有心跳和重连策略。对于 CORS 报错，先在浏览器开发者工具确认实际 `Origin`、预检响应与允许头，再检查服务端是否仍正确认证；不要为了消除浏览器错误而把所有源和头都放开。
