...
**端口**标识主机上的传输层端点，范围是 0 到 65535。服务器通常在固定端口监听，例如 HTTPS 常用 443；客户端在发起连接时通常由操作系统选择临时源端口。把“IP + 端口”合在一起，才足以把一条 TCP 连接的一端说清楚。在已经确定协议为 TCP 的语境中，一条连接由源 IP、源端口、目的 IP、目的端口的**四元组**区分；把传输协议也纳入描述时，就得到网络排障常说的**五元组**。因而同一台 Web 服务器可以同时服务许多客户端，即使它们都访问 `:443`：客户端的源地址或源端口不同。
...
sequenceDiagram
  ...
  Note left of S: ESTABLISHED after ACK arrives
```
