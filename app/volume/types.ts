...
function makeUnit(chapter: ChapterProfile, topic: TopicSeed, index: number): DeepUnit {
  ...
  const openings = [
    `先把 **${topic.title}** 当成一个会影响程序正确性的基础概念，而不是清单里的名词。${topic.definition}。这句话看起来朴素，但它决定了代码里什么可以被相信、什么必须先校验、什么变化需要留下记录。`,
  ...
  const misconceptions = [
    ...
    `不要落入这个误区：${topic.misconception}。基础教材里反复强调边界，不是为了堆工程话术，而是因为边界一旦模糊，错误就会在系统里漂移。`,
  ...
  const studyNotes = [
    [
      `作为基础知识，它的学习顺序应该很朴素：先看最小例子，再看真实系统里的变形，最后再回到常见概念。比如你可以先写一个只有十几行的小程序，让“${topic.title}”的正常情况和异常情况都发生一次。这样再读框架文档时，文档里的名词就不会悬在空中。`,
    ...
    [
      ...
      `在练习里，建议你故意制造一个反例。比如让输入为空、让依赖超时、让任务被取消、让同一个请求执行两次。反例会让抽象概念变得非常具体，也会让你意识到很多常见追问其实来自真实事故，而不是凭空编出来的题目。`,
  ...
  const reinforcementNotes = [
    [
      `从能力校准角度看，你可以把它记成一个问题：这里到底在保护什么？保护的是类型安全、资源释放、协议语义、数据一致性，还是用户权限。只要能说出保护对象，就不会只停留在背概念。`,
...
export function buildDeepChapter(profile: ChapterProfile): DeepChapter {
  return {
    ...
    preface: [
      ...
      `学完正文后，再进入本章的能力强化区，把基础概念转换成核心追问、手写题和系统设计取舍。这样学习顺序会更自然：先懂，再练，再接受追问。`,
